...
const gradeToPoints = [
    { min: 97, max: 100, points: 4.0 },
    { min: 93, max: 96, points: 4.0 },
    { min: 90, max: 92, points: 3.7 },
    { min: 87, max: 89, points: 3.3 },
    { min: 83, max: 86, points: 3.0 },
    { min: 80, max: 82, points: 2.7 },
    { min: 77, max: 79, points: 2.3 },
    { min: 73, max: 76, points: 2.0 },
    { min: 70, max: 72, points: 1.7 },
    { min: 67, max: 69, points: 1.3 },
    { min: 63, max: 66, points: 1.0 },
    { min: 60, max: 62, points: 0.7 },
    ...
    try {
        ...
        return {
            ...
            schedule: Array.isArray(saved.schedule)
                ? saved.schedule.map(course => ({
                    ...course,
                    teacher: String(course.teacher ?? "").trim()
                }))
                : [],

            homework: Array.isArray(saved.homework)
                ? saved.homework
                : [],

            bellMode: saved.bellMode === "hour"
                ? "hour"
                : "normal"
        };
...
function getFormData() {
    ...
    if (
        ...
        alert(
            "Please provide a class name, valid credit hours, and a grade from 0 to 100."
        );
...
function calculateGPA() {
    ...
    data.classes.forEach(course => {
        ...
        const gradePoints =
            gradeToPoints.find(
                range =>
                    course.grade >= range.min &&
                    course.grade <= range.max
            )?.points ?? 0;

        const weightedPoints =
            gradePoints +
            (classLevelAdjustment[course.classLevel] ?? 0);

        totalCredits += course.creditHours;

        unweighted +=
            gradePoints * course.creditHours;

        weighted +=
            weightedPoints * course.creditHours;
    });

    document.getElementById(
        "unweighted-gpa"
    ).textContent = totalCredits
        ? (unweighted / totalCredits).toFixed(2)
        : "0.00";

    document.getElementById(
        "weighted-gpa"
    ).textContent = totalCredits
        ? (weighted / totalCredits).toFixed(2)
        : "0.00";
}
        ...
        try {
            ...
            data.classes = imported
                ...
                .filter(course =>
                    ...
                    course.grade <= 100 &&
                    ["q1", "q2", "q3", "q4"]
                        .includes(course.quarter) &&
                    ["normal", "honors", "ap"]
                        .includes(course.classLevel)
                );
            ...
            alert(
                "Error importing classes. Please ensure the file is correctly formatted."
            );
        }
    };
    ...
    if (!tasks.length) {
        ...
        list.innerHTML = `
            <div class="task-empty">
                Chod do something today please...
            </div>
        `;
...
function populateHomeworkClasses() {
    ...
    const select =
        document.getElementById("homework-class");

    const current =
        select.value;

    const classNames =
        getClassNamesForHomework();
    ...
    if (classNames.includes(current)) {
...
function homeworkSort(a, b) {
    ...
    const completedCompare =
        Number(Boolean(a.done)) -
        Number(Boolean(b.done));
...
function renderHomework() {
    ...
    if (!sorted.length) {
        ...
        list.innerHTML = `
            <div class="homework-empty">
                No homework added yet. You are all caught up.
            </div>
        `;
    ...
    sorted.forEach(item => {
        ...
        row.className =
            `homework-item${item.done ? " completed" : ""}${overdue ? " overdue" : ""}`;
        ...
        row.innerHTML = `
            <div class="homework-main">
                ...
                <div class="homework-meta">
                    ...
                    <span class="homework-badge ${escapeHtml(item.priority)}">
                        ${escapeHtml(
    ...
    progress.textContent =
        `${homework.filter(item => item.done).length} / ${homework.length}`;
...
document
    ...
    .addEventListener("submit", event => {
        ...
        const name =
            document
                .getElementById("homework-name")
                .value
                .trim();

        const className =
            document
                .getElementById("homework-class")
                .value;

        const priority =
            document
                .getElementById("homework-priority")
                .value;

        const dueDate =
            document
                .getElementById("homework-due")
                .value;

        if (!name || !className || !dueDate) {

            alert(
                "Please enter the homework name, class, and due date."
            );
    ...
    if (!customSchedule.length) {
        ...
        container.innerHTML = `
            <div class="schedule-empty">
                No classes added yet. Your schedule is a clean slate.
            </div>
        `;
        ...
        row.innerHTML = `
            <div>
                ...
                <small>
                    ${course.days.join(", ")}
                    ·
                    ${formatStoredTime(selectedStart)}
                    –
                    ${formatStoredTime(selectedEnd)}
                    ${course.bellAffected
                        ? " · Bell-specific"
                        : ""}
                </small>
...
document
    ...
    .addEventListener("submit", event => {
        ...
        if (
            ...
            alert(
                "Enter valid bell-specific start and end times."
            );
...
function updateTimeRemaining() {
    ...
    todaySchedule.forEach(course => {
        ...
        if (current) {
            ...
            remaining =
                `${end - currentMinutes} min remaining`;

        } else if (currentMinutes < start) {

            remaining =
                `${start - currentMinutes} min until start`;
        }
        ...
        row.className =
            `schedule-item${current ? " current-class" : ""}`;

        row.innerHTML = `
            <div class="schedule-class-name">
            ...
            <div class="schedule-time">
                ${formatStoredTime(course.startTime)}
            </div>

            <div class="schedule-time">
                ${formatStoredTime(course.endTime)}
            </div>
...
const compliments = [
    ...
    "Imagine being bad at SigFigs?",

    "Imagine being a NEERRRD!",

    "You are the reason soap has instructions.",

    "Your only two brain cells are fighting for last place.",

    "You are not locked in.",

    "Why you smiling, ain't nothing funny here?",
